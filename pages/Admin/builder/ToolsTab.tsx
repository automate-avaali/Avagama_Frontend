...
const ToolsTab: React.FC<ToolsTabProps> = ({ agentId }) => {
  ...
  const openSlackConfig = (existing?: StandaloneTool) => {
    if (existing) {
      setConfiguringTool({
        ...existing,
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
               {[
                 ...
                 <div key={idx} className="p-6 bg-white border border-gray-100 rounded-[32px] flex items-center justify-between group hover:border-[#a26da8] transition-all">
                    ...
                    <button 
                      onClick={() => tc.isSlack ? openSlackConfig() : toast('Integration coming soon')}
              ...
              <div key={tool._id} className="group flex flex-col md:flex-row items-start md:items-center justify-between p-7 bg-white border border-gray-100 rounded-[32px] hover:border-[#a26da8] hover:shadow-2xl hover:shadow-purple-50 transition-all duration-300">
                 <div className="flex items-center gap-6 flex-1">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${tool.enabled ? 'bg-purple-100 text-[#a26da8]' : 'bg-gray-100 text-gray-400'}`}>
                       {tool.name.toLowerCase().includes('slack') ? <MessageSquare size={24} /> : tool.type === 'zapier' ? <Zap size={24} /> : tool.type === 'mcp' ? <Puzzle size={24} /> : <Box size={24} />}
                    </div>
                 ...
                 <div className="flex items-center gap-8 mt-4 md:mt-0">
                    ...
                    <div className="flex items-center gap-2">
                       <button 
                        onClick={() => tool.name.toLowerCase().includes('slack') ? openSlackConfig(tool) : toast('Advanced config coming soon')}
          ...
          <div className="relative w-full max-w-xl bg-white rounded-[40px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
             <div className="p-8 border-b border-gray-50 flex items-center justify-between">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      <h4 className="text-lg font-black text-gray-900">Configure Slack</h4>
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Built-in Notification Tool</p>
                   </div>
                </div>
             ...
             <div className="p-8 space-y-6">
                ...
                <div className="space-y-2">
                   ...
                   />
                   <p className="text-[9px] font-bold text-gray-400 px-1">Describe escalation triggers precisely (min 30 chars).</p>
                </div>
