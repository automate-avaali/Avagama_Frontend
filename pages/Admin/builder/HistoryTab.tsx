...
const HistoryTab: React.FC<HistoryTabProps> = ({ agentId, refreshKey }) => {
  ...
  useEffect(() => {
    // If a session is currently selected, refresh its messages too
    const currentId = selectedSession?._id || selectedSession?.session_id;
    if (currentId && refreshKey && refreshKey > 0) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const currentSelectedId = selectedSession?._id || selectedSession?.session_id;
      ...
      fetchSessions(); // Revert list on error
    } finally {
  ...
  return (
    <>
           ...
           {selectedSession ? (
             <>
                <div className="p-6 border-b border-gray-50 flex items-center justify-between">
                   ...
                   <div className="flex items-center gap-2">
                      <button onClick={() => setConfirmDeleteId(selectedSession._id || selectedSession.session_id)} className="p-3 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all">
                         <Trash2 size={18} />
                      </button>
                   </div>
                </div>
      ...
      {confirmDeleteId && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-[32px] p-8 max-w-sm w-full shadow-2xl border border-gray-100 animate-scaleUp">
            <div className="w-16 h-16 bg-red-50 text-red-500 rounded-2xl flex items-center justify-center mb-6">
              <Trash2 size={32} />
            </div>
            <h3 className="text-xl font-black text-gray-900 mb-2">Delete Log?</h3>
            ...
            <div className="flex gap-3">
              <button 
                ...
                className="flex-1 py-4 bg-gray-50 text-gray-500 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-100 transition-all"
              >
                Cancel
              </button>
              <button 
